...
import org.flintparticles.twoD.zones.*;
  ...
  import org.flintparticles.common.energyEasing.Bounce;
	...
	public class InvisiBrick extends MovieClip  {
		...
		*/
		private var isValid:Boolean = false; //not used presently
		private var isRendered:Boolean = false; //not used presently
		private var isListening:Boolean = false; //not used presently
		private var isActivated:Boolean = false; //Used to track the activation state of the IB
		private var matrix:BitmapData;
		private var isInit:Boolean = true; //Used to track the phase of IB life
		private var isChild:Boolean = false; //Used to indicate whether opr nto this IB is a child of something yet
		private static const COLOR_RED_POWER:int = 1; //Constants for brickID
		private static const COLOR_GREEN:int = 2;
		private static const COLOR_BLUE_BLOCK:int = 3;
		private var hRect:Rectangle; //Handle to this IB's rectangle bitmapdata
		private var hBData:BitmapData; //handle to this IB's bitmap data
		
		//Special stuff, like elemental properties and (maybe) particles
		//private var ex:Explode = new Explode() //for now we'll try just one Explode object, ball.ex
		
		public function InvisiBrick(enPaddle:enemyPaddle = null,paddle:MovieClip= null,ball:MovieClip= null,theBall:Ball= null,ePaddle:MovieClip= null,startB:Button= null,myX:Number= 0,myY:Number= 0,level:Array= null,bricksH:Bricks= null,stageR:Stage = null,mainR:FlashForceMain= null) {
			//Constructor code
			...
			addEvent();
			*/
			//bricksHandle.setActualStageSize(bricksHandle.getActualStageSize() + 1);
			//this.addEventListener(Event.ENTER_FRAME, queryVisible);

		}
		public function queryVisible(event:Event){
			//Below is DEBUG info specifically for IB rendering
			/*
			trace("this.visible is: " + this.visible);
			trace("this.x returns: " + this.x + ", and this.y returns: " + this.y);
			trace("this.width is: " + this.width);
			trace("this.height is: " + this.height);
			*/
			
			//Below is DEBUG info for system architecture
			/*
			trace("stage size: " + bricksHandle.stageSize + "actual stage size " + bricksHandle.getActualStageSize() + " block bricks remaining: " + bricksHandle.brickBlockCount + "bricks 'destroyed': " + ball.bricksDestroyed);
			*/
		}
		
		public function getIsChild():Boolean{
			return this.isChild;
		}
		public function setIsChild(b:Boolean){
			this.isChild = b;
		}
		...
		public function setStarHit(){
			...
			else if (this.x >= 55 && this.x < 65){
				starHit = 1;
			}
			else if (this.x >= 110 && this.x < 120){
				starHit = 2;
			}
			else if (this.x >= 165 && this.x < 175){
				starHit = 3;
			}
			else if (this.x >= 215 && this.x < 225){
				starHit = 4;
			}
			else if (this.x >= 270 && this.x < 280){
				starHit = 5;
			}
			else if (this.x >= 325 && this.x < 335){
				starHit = 6;
			}
			else if (this.x >= 380 && this.x < 390){
				starHit = 7;
			}
			else if (this.x >= 415 && this.x < 470){
				starHit = 8;
			}
			else if (this.x >= 470 && this.x < 500){
				starHit = 9;
			}
		}
		...
		public function setInert(){
			if (brickID == "red"){
				...
				if (brickID == "block"){
				  ...
				  bricksHandle.brickBlockCount--; //dec brickBlockCount
				}
				deathTimer.stop();
				deathTimer.reset();
				...
				//inc ball.bricksDestroyed iff this function is not being invoked by bricksHandle.genLevel()
				if(!bricksHandle.getIsNewLevel()){
				...
				//report controls for DEBUG
				//trace("ball.bricksDestroyed: " + ball.bricksDestroyed + ", bricksHandle.stageSize: " + bricksHandle.stageSize + ", bricksHandle.brickBlockCount: " + bricksHandle.brickBlockCount);
				
				//if this is the last non-block brick OR the last brick in the stage,
				//call checkLevel in Bricks
				if (ball.bricksDestroyed >= (bricksHandle.stageSize-bricksHandle.brickBlockCount) && ball.bricksDestroyed <= bricksHandle.stageSize){
					bricksHandle.checkLevel();
				}
...
/*shouldn't need this one in new way
		public function selfDestruct3(){ //need waitToDie methods for each brick type.  So far have waitToDieR...
...
*/
		...
		/*//Function not used presently... see Explode class
		...
		*/
		...
		public function death(event:TimerEvent){
			if (ball.killAll == 0){
				...
				////if (!mainRef.isPaused){
			//ball.ex = new Explode(this.x,this.y,myColor);
			...
			//Thus it is desirable to have each existing invisibrick hook an 
			//eventlistener to ball.startButton 
			//Question is: should we later remove them or leave them hooked up with the new cache and 'release' architecture?
		...
		public function returnARGB(rgb:uint, newAlpha:uint):uint{
		...
		public function activateBrick(brickColor:String){
			...
			if (!isInit){ //should only run after the first activation
				this.width = 55;
				this.height = 15;
			}
			...
			if (brickColor == "red"){
			    ...
			    */
				if (isInit){
					/*
					//Single color fill method
				  
			      this.graphics.beginFill(0xFF0000);
				  //matrix = new BitmapData(25,25);
				 ...
				 ///-->look into this for re-rendering... this.bitmapData.fillRect(this.bitmapData.rect,0);
			      this.graphics.endFill();
				  */
				  
				  //Bitmap fill method
				  
				  //Instantiate bitmapdata for this IB
				  hBData = new BitmapData(55,15,false,0xFFFF0000);
				 
				  //Get a handle on the generated bitmapdata rectangle
				  //MUST ONLY BE USED IF isInit IS ONLY TRUE ONCE, AS IS THE CASE NOW
				  hRect = hBData.rect;
				  
				  //Perform the actual rendering
				  hBData.draw(bricksHandle.getRedBrickData());
				  this.graphics.beginBitmapFill(hBData);
				  this.graphics.drawRect(0,0,55,15);
				  this.graphics.endFill();
					...
					/*
					//Clear the bitmapdata fill by writing a transparent fill to it
					hBData.fillRect(hRect,0);
					
					//Create a new fill for this color and use it
					//this.graphics.beginFill(0x11ff0000);
					hBData.fillRect(hRect,0xFFFF0000);
					*/
			...
			if (brickColor == "blue"){
				...
				*/
				...
				//Instantiate bitmapdata for this IB
				  ...
				  //Perform the actual rendering
				  hBData.draw(bricksHandle.getBlueBrickData());
				  this.graphics.beginBitmapFill(hBData);
				  this.graphics.drawRect(0,0,55,15);
				  this.graphics.endFill();
					...
					//Create a new fill for this color and use it
					//this.graphics.beginFill(0x11ff0000);
					hBData.fillRect(hRect,0xFF0000FF);
					*/
			...
			if (brickColor == "green"){
				...
				*/
				...
				//Instantiate bitmapdata for this IB
				  ...
				  //Perform the actual rendering
				  hBData.draw(bricksHandle.getGreenBrickData());
				  this.graphics.beginBitmapFill(hBData);
				  this.graphics.drawRect(0,0,55,15);
				  this.graphics.endFill();
					...
					//Create a new fill for this color and use it
					//this.graphics.beginFill(0x11ff0000);
					hBData.fillRect(hRect,0xFF00FF00);
					*/
				...
				//BitmapData.rect //THIS is what returns a new rectangle object each time
				//finally add the onEnterFrame callback listener
			...
			if (brickColor == "power"){
				...
				*/
				...
				//Instantiate bitmapdata for this IB
				  ...
				  //Perform the actual rendering
				  hBData.draw(bricksHandle.getPowerBrickData());
				  this.graphics.beginBitmapFill(hBData);
				  this.graphics.drawRect(0,0,55,15);
				  this.graphics.endFill();
					...
					//Create a new fill for this color and use it
					//this.graphics.beginFill(0x11ff0000);
					hBData.fillRect(hRect,0xFFAAFF00);
					*/
			...
			if (brickColor == "block"){
				...
				*/
				...
				//Instantiate bitmapdata for this IB
				  ...
				  //Perform the actual rendering
				  hBData.draw(bricksHandle.getBlockBrickData());
				  this.graphics.beginBitmapFill(hBData);
				  this.graphics.drawRect(0,0,55,15);
				  this.graphics.endFill();
					...
					//Create a new fill for this color and use it
					//this.graphics.beginFill(0x11ff0000);
					hBData.fillRect(hRect,0xFF000000);
					//this.graphics.endFill();
					*/
			...
			//bricksHandle.setIsInit(false); //cause only one (first) brick to be rendered...
			this.isInit = false; 
		...
		public function getActivatedState():Boolean{
			return this.isActivated;
		}
		
		//Specialized functions for each brick type follow...
		
		//BECOME A RED BRICK
		
		public function waitToDieR(event:Event){
			//trace("a red is listening");
			...
			*/
			if (ball.killAll == 1){
				//selfDestruct2(); //old way
				setInert(); //new way
			}
			/*we don't use evilsight anymore
